...
class PointCHR extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - nillable: true
     * @var \StructType\ListeHoraireOuverturePourUnJour[]
     */
     ...
     * - minOccurs: 0
     * - nillable: true
     * @var \StructType\PeriodeFermeture[]
     */
     ...
     * @param \StructType\ListeHoraireOuverturePourUnJour[] $listeHoraireOuverture
     * @param \StructType\PeriodeFermeture[] $listePeriodeFermeture
    ...
    public static function validateListeHoraireOuvertureForArrayConstraintsFromSetListeHoraireOuverture(?array $values = []): string
    {
        ...
        foreach ($values as $pointCHRListeHoraireOuvertureItem) {
            ...
            if (!$pointCHRListeHoraireOuvertureItem instanceof \StructType\ListeHoraireOuverturePourUnJour) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The listeHoraireOuverture property can only contain items of type \StructType\ListeHoraireOuverturePourUnJour, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \StructType\ListeHoraireOuverturePourUnJour $item
     * @return \StructType\PointCHR
     */
    public function addToListeHoraireOuverture(\StructType\ListeHoraireOuverturePourUnJour $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\ListeHoraireOuverturePourUnJour) {
            throw new InvalidArgumentException(sprintf('The listeHoraireOuverture property can only contain items of type \StructType\ListeHoraireOuverturePourUnJour, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
     ...
     * this property may have been unset before, due to the fact that this property is
     * removable from the request (nillable=true+minOccurs=0)
     * @return \StructType\PeriodeFermeture[]
     */
    ...
    public static function validateListePeriodeFermetureForArrayConstraintsFromSetListePeriodeFermeture(?array $values = []): string
    {
        ...
        foreach ($values as $pointCHRListePeriodeFermetureItem) {
            // validation for constraint: itemType
            if (!$pointCHRListePeriodeFermetureItem instanceof \StructType\PeriodeFermeture) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The listePeriodeFermeture property can only contain items of type \StructType\PeriodeFermeture, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \StructType\PeriodeFermeture $item
     * @return \StructType\PointCHR
     */
    public function addToListePeriodeFermeture(\StructType\PeriodeFermeture $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\PeriodeFermeture) {
            throw new InvalidArgumentException(sprintf('The listePeriodeFermeture property can only contain items of type \StructType\PeriodeFermeture, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
