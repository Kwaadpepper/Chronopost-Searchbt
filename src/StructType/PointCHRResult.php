...
class PointCHRResult extends AbstractStructBase
{
     ...
     * @param \StructType\PointCHR[] $listePointRelais
     ...
     * Set errorCode value
     * @param int $errorCode
     * @return \StructType\PointCHRResult
     */
    ...
    public static function validateListePointRelaisForArrayConstraintsFromSetListePointRelais(?array $values = []): string
    {
        ...
        foreach ($values as $pointCHRResultListePointRelaisItem) {
            ...
            if (!$pointCHRResultListePointRelaisItem instanceof \StructType\PointCHR) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The listePointRelais property can only contain items of type \StructType\PointCHR, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \StructType\PointCHR $item
     * @return \StructType\PointCHRResult
     */
    public function addToListePointRelais(\StructType\PointCHR $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\PointCHR) {
            throw new InvalidArgumentException(sprintf('The listePointRelais property can only contain items of type \StructType\PointCHR, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
