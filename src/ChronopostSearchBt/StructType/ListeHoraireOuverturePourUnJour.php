...
class ListeHoraireOuverturePourUnJour extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - nillable: true
     * @var \ChronopostSearchBt\StructType\HoraireOuverture[]
     */
     ...
     * @param int $jour
     * @param \ChronopostSearchBt\StructType\HoraireOuverture[] $listeHoraireOuverture
     */
    ...
    public static function validateListeHoraireOuvertureForArrayConstraintFromSetListeHoraireOuverture(?array $values = []): string
    {
        ...
        foreach ($values as $listeHoraireOuverturePourUnJourListeHoraireOuvertureItem) {
            ...
            if (!$listeHoraireOuverturePourUnJourListeHoraireOuvertureItem instanceof \ChronopostSearchBt\StructType\HoraireOuverture) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The listeHoraireOuverture property can only contain items of type \ChronopostSearchBt\StructType\HoraireOuverture, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \ChronopostSearchBt\StructType\HoraireOuverture $item
     * @return \ChronopostSearchBt\StructType\ListeHoraireOuverturePourUnJour
     */
    public function addToListeHoraireOuverture(\ChronopostSearchBt\StructType\HoraireOuverture $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \ChronopostSearchBt\StructType\HoraireOuverture) {
            throw new InvalidArgumentException(sprintf('The listeHoraireOuverture property can only contain items of type \ChronopostSearchBt\StructType\HoraireOuverture, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
